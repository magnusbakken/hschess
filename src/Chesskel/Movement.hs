...
Description : Functions for chess moves.
...
data MoveContext = MkMoveContext {
    -- |The cell the piece moved from. Equivalent to the first cell in the
    --  'Move' type.
    --
    ...
    -- |The cell the piece moved to. Equivalent to the second cell in the
    --  'Move' type.
    --
    --  If the move is a castling move, there are actually two pieces that are
    --  moving. In those cases the 'castlingData' must be perused as well.
    ...
    --  captured pawn is on.
    --
    --  This is not to be confused with the 'previousEnPassantCell' field on
    --  the 'PositionContext', which is the cell the capture can be made at.
    --  For instance, if white moves a pawn from 'e2' to 'e4', the
    --  'PositionContext' will contain 'e3'. If black then captures en passant
    --  on 'e3', this field will have a value of 'e4'.
...
data MoveError =
    ...
    --  cell, e.g. if an attempt is made to move from 'a1' to 'f8', or if the
    --  move could be made except for there being something blocking the
    --  movement. This could be either due to a friendly piece being in the
    --  destination cell, or a friendly or enemy piece in any of the
    --  intermediate cells.
    ...
    -- 
    --  This is only applicable when 'Chesskel.Movement.Minimal.makeMinimalMove'
    --  (or 'Chesskel.Gameplay.playMinimalMove') is used. A list of possible
    --  source cells is included with the error.
...
--
--  This is just a curried version of the standard 'Move' constructor,
...
findAllLegalMoves pc =
    piecesOfColor (currentPlayer pc) (position pc) >>= findLegalMoves pc
