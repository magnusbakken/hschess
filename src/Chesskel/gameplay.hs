...
data HeaderData = HD {
    ...
    --  When this is a physical location it should preferably be on the format City, Region COUNTRY.
    siteHeader :: String,
    
    -- |The date the game was played. Should preferably be on the format YYYY.MM.DD.
    dateHeader :: String,
    
    -- |The playing round within the event. Should preferably be an integer.
    roundHeader :: String,
    
    -- |The name of the white player. Should preferably be on the format Last Name, First Name.
    whiteHeader :: String,
    
    -- |The name of the black player. Should preferably be on the format Last Name, First Name.
    blackHeader :: String,
    
    -- |The result of the game. If the result is unknown, 'Ongoing' should be used.
    resultHeader :: Result
} deriving (Eq)
...
data ExtraHeader = EH {
    ...
    --  even if they're escaped with backslashes. Values containing quotation marks will cause
    --  PGN files generated from the header to be invalid.
    headerValue :: String
} deriving (Eq)

-- |Header data consisting of the Seven Tag Roster and an optional list of extra headers.
...
data GameContext = GC {
    -- |The current position. Should always be the same as the last item in the 'positions' list.
    currentPosition :: PositionContext,
    
    -- |The list of positions in the game. Should never be empty. The first position in the list
    --  should be the starting position, and the last position should be the same as 'currentPosition'.
    positions :: [PositionContext],
    
    -- |The list of moves in the game. The positions and moves should be interleaved, in the
    --  sense that the first position comes before the first move, then comes the first move,
    --  then the second position, etc. The last move should always lead to the last position,
    --  except when there are no moves at all. The 'positions' list should therefore always
    ...
    -- |The result of the game. Should only be changed using the 'resign', 'makeDraw' and 'setResult'
    --  functions. Doing so will ensure that the result in the header data is kept in sync.
    gameResult :: Result
} deriving (Eq)
...
-- |Used when starting a standard game. All the headers in the SRT will be set to
--  a value indicating that they're unknown, and the result will be set as 'Ongoing'.
...
-- |Makes the player of the given color resign. Equivalent to setting the result to 'WhiteWin'
--  if the given color is 'Black', or 'BlackWin' if the given color is 'White'.
